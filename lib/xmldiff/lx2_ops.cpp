...
int loadXmlFile(string filename, string alias, const struct globalOptions & options)
{
    ...
    doc = xmlCtxtReadFile(ctxt, 
                          filename.c_str(), 
                          NULL, 
                          (options.cleanText)?XML_PARSE_NOBLANKS:0 /*+ XML_PARSE_RECOVER*/);
    if (!doc) 
    { 
        throwError(XD_Exception::XDE_READ_ERROR, "Error loading %s : %s", filename.c_str(), ctxt->lastError.message); 
    }
