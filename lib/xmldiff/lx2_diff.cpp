...
int diffNode(xmlNodePtr nodeBefore, xmlNodePtr nodeAfter, const struct xmldiff_options & options)
{
    ...
    // if (fgVerbose) printf("\n[Node] %s", StrX(getNodePath(nodeAfter).c_str()));
    ...
    while ((iterBefore != listBefore->end()) || (iterAfter != listAfter->end()))
    {
        ...
        default:
            ...
            iterAfter->node->_private = iterBefore->node;
            iterBefore->node->_private = iterAfter->node;
    ...
    if ( (afterNodesNb != 0) || (beforeNodesNb != 0))
    {
         ...
         if (curPercent != precPercent)
         {
             ...
             {
                 options.callbackProgressionPercent(curPercent, precPercent, beforeNodesNb, afterNodesNb, nodesCur);
             }
...
int diffTree(xmlNodePtr nodeBefore, xmlNodePtr nodeAfter, const struct xmldiff_options & options)
{
    ...
    if (options.callbackProgressionPercent != NULL)
    {
        ...
        options.callbackProgressionPercent(-1, -1, beforeNodesNb, 0, 0);
        afterNodesNb = countElementNodes(nodeAfter);
        options.callbackProgressionPercent(-2, -2, beforeNodesNb, afterNodesNb, 0);
    }
    ...
    if (options.callbackProgressionPercent != NULL)
    {
        options.callbackProgressionPercent(100, precPercent, beforeNodesNb, afterNodesNb, nodesCur); precPercent = 100;
        options.callbackProgressionPercent(-3, -3, beforeNodesNb, afterNodesNb, nodesCur);
    }
    return status;
}
